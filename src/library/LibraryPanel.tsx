...
import { EpisodeAssemblerDialog } from '../documentary/EpisodeAssemblerDialog';
import type { EpisodeAssemblyRequest, EpisodeAssemblyResult } from '../documentary/episodeAssembler';
...
interface LibraryPanelProps {
  ...
  onAssembleEpisode: (request: EpisodeAssemblyRequest) => EpisodeAssemblyResult;
...
export function LibraryPanel({ semanticScopeId, templates, onAddTemplate, onAddAudio, playerRef, fps, items, sequenceOptions, onAddSequence, trackOptions, captionTracks, onSetCaptions, onCreateCaptionTrack, onUpdateCaptions, onSetItemTranscript, onToggleWord, onCleanScript, onSetGapCap, onSetTranscriptPlayOrder, onReorderTrackItems, onClearEdits, assets, mediaFolders, usedAssetIds, offlineAssetIds, onAssetLoadError, onImportMedia, onImportMobileMedia, onIngestDirectoryAsset, onTranscribeAsset, onAddMediaItem, onAddMediaAssetsToTimeline, onAssembleEpisode, onUseMediaAI, onCreateMediaFolder, onRenameMediaFolder, onDeleteMediaFolder, onMoveMediaAssets, onRenameMediaAsset, onRenameMediaAssets, onSetMediaAssetFavorite, onSetMediaAssetsFavorite, onRemoveMediaAsset, onRemoveMediaAssets, onPasteMediaAssets, onRelinkMediaAsset, creativeMode, onCreativeModeChange, onAddSolid, onUseTemplateAI, selectedItem, onApplyTransition, onApplyFx, onApplyZoom }: LibraryPanelProps) {
  ...
  const {
    activeWatch: activeDirectoryWatch,
    available: directoryImportAvailable,
    busy: directoryImportBusy,
    start: startDirectoryImport,
  } = directoryImport;
  const autoImportHandled = useRef(false);
  const autoOpenedWatch = useRef<string | null>(null);
  useEffect(() => {
    if (autoImportHandled.current || !directoryImportAvailable) return;
    const key = 'editor-documental:auto-import-project';
    if (sessionStorage.getItem(key) !== semanticScopeId) return;
    autoImportHandled.current = true;
    sessionStorage.removeItem(key);
    void startDirectoryImport();
  }, [directoryImportAvailable, semanticScopeId, startDirectoryImport]);
  useEffect(() => {
    const watch = activeDirectoryWatch;
    if (!watch || directoryImportBusy || !watch.srtFiles.length || !assets.length
      || autoOpenedWatch.current === watch.watchId) return;
    autoOpenedWatch.current = watch.watchId;
    setEpisodeAssemblerOpen(true);
  }, [activeDirectoryWatch, assets.length, directoryImportBusy]);
    ...
    <section className="cc-library-panel">
      ...
      {extensionOpen ? (
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, borderTop: `0.5px solid ${theme.border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '9px 11px', borderBottom: `0.5px solid ${theme.border}`, background: theme.panelAlt }}>
            <button type="button" onClick={() => setEpisodeAssemblerOpen(true)}
              style={{ border: `0.5px solid ${theme.accent}`, borderRadius: 5, background: theme.accent, color: theme.onAccent, padding: '7px 11px', fontSize: 11.5, fontWeight: 700, cursor: 'pointer' }}>
              Montar episódio
            </button>
            <span style={{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: theme.textDim, fontSize: 10.5 }}>
              {directoryImport.activeWatch ? `Pasta: ${directoryImport.activeWatch.directoryName}` : 'Importe a pasta e escolha o SRT'}
            </span>
          </div>
      ...
      {episodeAssemblerOpen && <EpisodeAssemblerDialog
        assets={assets}
        fps={fps}
        initialSrtFiles={directoryImport.activeWatch?.srtFiles ?? []}
        hasTimelineContent={items.length > 0}
        onApply={onAssembleEpisode}
        onClose={() => setEpisodeAssemblerOpen(false)}
      />}
    </section>
  );
}
