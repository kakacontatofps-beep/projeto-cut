...
export function BrandMark({ size = 16 }: { size?: number }) {
  return (
    <span className="cc-brand-mark" aria-hidden style={{ width: size, height: size, fontSize: Math.max(7, size * 0.38) }}>KC</span>
  );
}

/** Fork word mark. The historical function name stays stable for upstream mergeability. */
export function OpenChatCutWordmark({ width = 126 }: { width?: number }) {
  return (
    ...
    >
      ...
      >
        <tspan x="42" y="80" fill="currentColor" fontSize="64" fontWeight="820" letterSpacing="-0.045em">
          Kaka
        </tspan>
        <tspan x="218" y="80" fill="var(--cc-accent)" fontSize="64" fontWeight="760" letterSpacing="-0.045em">
          Cut
        </tspan>
      </text>
    </svg>
