...
export function useDashboardActions(refresh: () => Promise<void>): DashboardActions {
  ...
  return {
    ...
    onNew: async () => {
      ...
      await refresh();
      // The documentary workflow starts with a media folder. LibraryPanel consumes
      // this one-shot marker after the editor is mounted and opens the native picker.
      sessionStorage.setItem('editor-documental:auto-import-project', project.id);
