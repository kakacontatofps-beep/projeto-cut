...
export interface ActiveDirectoryWatch {
  ...
  readonly srtFiles: readonly { name: string; content: string }[];
}
...
export class DirectoryImportRuntime {
  ...
  get activeWatch(): ActiveDirectoryWatch | null {
    ...
    return session ? { watchId: session.watchId, directoryName: session.directoryName, srtFiles: session.srtFiles } : null;
  }
  ...
  #createSession(result: DirectoryWatchStartResult, hashes: readonly string[]): RuntimeSession {
    ...
    this.#options.onWatchChange({ watchId: session.watchId, directoryName: session.directoryName, srtFiles: session.srtFiles });
    return session;
  }
