...
function readInitial(): Locale {
  try {
    ...
    return stored === 'en' || stored === 'zh' || stored === 'pt' ? stored : 'pt';
  } catch {
    return 'pt';
  }
}
...
export function setLocale(next: Locale): void {
  ...
  document.documentElement.lang = next === 'pt' ? 'pt-BR' : next === 'en' ? 'en' : 'zh-CN';
...
export function t(zh: string, params?: Record<string, string | number>): string {
  const raw = current === 'zh' ? zh : current === 'pt' ? (PT[zh] ?? EN[zh] ?? zh) : (EN[zh] ?? zh);
