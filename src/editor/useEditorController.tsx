...
import { assembleEpisodeDocument } from '../documentary/episodeAssembler';
...
import { isAudioTransition, type ProjectDoc, type TransitionType } from './types';
...
function buildEditorWorkspaceViewProps(
  ...
  const applyTransitionFromLibrary = (
    type: TransitionType,
    ...
    if (!selectedId) {
      showAppToast(t('请在时间线上选择一个 B-roll'), { error: true });
      return;
    }
    const incomingId = isAudioTransition(type)
      ? selectedId
      : transitionIncomingForSelectedBroll(snapshot.items, selectedId);
    if (!incomingId) {
      showAppToast(t('这个 B-roll 附近没有相邻切点'), { error: true });
      return;
    }
    commands.addTransition(incomingId, type, undefined, custom);
    showAppToast(t('转场已应用到切点'));
  };
  const applyTransitionToSelectedTrack = (
    type: TransitionType,
    custom?: { frag: string; uniforms: Record<string, number>; label: string },
  ): number => {
    const snapshot = stateRef.current;
    const selected = snapshot.items.find((item) => item.id === snapshot.selectedId);
    if (!selected || isAudioTransition(type)) return 0;
    const targets = brollTransitionTargets(snapshot.items, selected.track);
    if (targets.length === 0) {
      showAppToast(t('当前轨道没有相邻切点'), { error: true });
      return 0;
    }
    commands.batch(targets.map((incomingItemId) => ({
      type: 'addTransition' as const,
      id: `tr_${crypto.randomUUID()}`,
      incomingItemId,
      transType: type,
      custom,
    })), 'Apply B-roll transitions');
    showAppToast(t('转场已应用到 {n} 个切点', { n: targets.length }));
