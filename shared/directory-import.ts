...
/** Small text sidecars discovered beside imported media. Paths are never exposed. */
export interface DirectorySrtFile {
  readonly name: string;
...
export interface DirectoryWatchStartResult {
  ...
  readonly srtFiles?: readonly DirectorySrtFile[];
}
...
export function isDirectoryWatchStartResult(value: unknown): value is DirectoryWatchStartResult {
  ...
  const srtFiles = result.srtFiles;
  return isDirectoryImportOpaqueId(result.watchId)
    ...
    && (srtFiles === undefined || (Array.isArray(srtFiles)
      && srtFiles.length <= 8
      && srtFiles.every((file) => file
        && typeof file === 'object'
        && !exposesRawDirectoryPath(file)
        && typeof file.name === 'string'
        && file.name.length > 0
        && file.name.length <= 512
