...
assert.equal(isDirectoryWatchStartResult({
  ...
  directoryName: 'Shots',
  files: [{ ...base, compatibilityNormalized: true }],
  srtFiles: [{ name: 'roteiro.srt', content: '1\n00:00:00,000 --> 00:00:01,000\nCena' }],
}), true);
assert.equal(isDirectoryWatchStartResult({
  watchId: 'watch-a', projectId: 'project-a', directoryName: 'Shots', files: [],
  srtFiles: [{ name: 'roteiro.srt', content: '', path: 'C:\\private\\roteiro.srt' }],
}), false, 'SRT sidecars must not expose a raw path');
