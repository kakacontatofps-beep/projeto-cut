...
set "ED_NODE=C:\Users\kaina\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe"
set "ED_NPM=C:\Program Files\nodejs\node_modules\npm\bin\npm-cli.js"

if not exist "%ED_NODE%" (
...
if not exist "%ED_NPM%" (
  echo O npm nao foi encontrado em %ED_NPM%.
  pause
  exit /b 1
)
...
if not exist "node_modules\electron\dist\electron.exe" (
  ...
  "%ED_NODE%" "%ED_NPM%" install
  if errorlevel 1 goto :error
)

if not exist "dist\index.html" (
  echo Preparando a interface do Kaka Cut...
  "%ED_NODE%" "%ED_NPM%" run build
  if errorlevel 1 goto :error
)

"%ED_NODE%" "%ED_NPM%" run desktop:dev:shared
exit /b %errorlevel%
