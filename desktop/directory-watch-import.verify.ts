...
const ROOT = resolve('watch');
const SOURCE = join(ROOT, 'clip.mov');
const UPLOADS = resolve('uploads');
const CHANGED_UPLOADS = resolve('changed-uploads');
const cleanupSet = (directory = UPLOADS, base = 'id') => new Set([
  join(directory, `${base}.mov`),
  join(directory, `${base}.mp4`),
  join(directory, `${base}.alpha.webm`),
]);
...
const escaped = harness({ realpath: async () => resolve('outside', 'clip.mov') });
assert.deepEqual(
...
const destinationChanged = harness({
  uploadDirectory: () => CHANGED_UPLOADS,
  canonicalUploadDirectory: async () => (++destinationChecks === 1 ? UPLOADS : CHANGED_UPLOADS),
});
await assert.rejects(
...
assert.deepEqual(
  ...
  new Set([...cleanupSet(), ...cleanupSet(CHANGED_UPLOADS)]),
...
assert.ok(secondBatch.removed.includes(join(UPLOADS, 'second.mov')));
