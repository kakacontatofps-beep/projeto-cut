...
export interface DirectoryWatchDependencies {
  ...
  readonly readSrtText?: (path: string) => Promise<string | null>;
}
...
async function discoverSrtFiles(
  root: string,
  dependencies: DirectoryWatchDependencies,
  cancelled: () => boolean,
): Promise<Array<{ name: string; content: string }>> {
  if (!dependencies.readSrtText) return [];
  const candidates = await scanImportDirectory(root, dependencies, cancelled);
  const output: Array<{ name: string; content: string }> = [];
  for (const candidate of candidates) {
    if (output.length >= 8 || extname(candidate.name).toLowerCase() !== '.srt') continue;
    const canonical = await dependencies.realpath(candidate.path);
    if (cancelled() || !isPathInside(root, canonical)) continue;
    const content = await dependencies.readSrtText(canonical);
    if (content !== null) output.push({ name: candidate.name, content });
  }
  return output.sort((left, right) => left.name.localeCompare(right.name, undefined, { numeric: true }));
}
...
export class DirectoryWatchSession {
  ...
  async start(): Promise<DirectoryWatchStartResult> {
    ...
    try {
      ...
      this.assertReady();
      const srtFiles = await discoverSrtFiles(this.options.root, this.dependencies, this.cancelled);
      this.phase = 'inactive';
      return {
