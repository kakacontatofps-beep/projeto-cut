...
const sidecar = createHarness(async (request) => request.name.endsWith('.srt')
  ? { status: 'unsupported', fingerprint: FINGERPRINT }
  : imported(request));
sidecar.tree.set(ROOT, [entry('001.jpg'), entry('roteiro.srt')]);
const sidecarSession = new DirectoryWatchSession({
  watchId: 'srt-sidecar', projectId: 'project-srt-sidecar', root: ROOT,
  pinnedUploadDirectory: UPLOADS, existingContentHashes: [], onImported: () => true,
}, {
  ...sidecar.dependencies,
  readSrtText: async (path) => path.endsWith('roteiro.srt')
    ? '1\n00:00:00,000 --> 00:00:01,000\nCena documental'
    : null,
});
const sidecarStart = await sidecarSession.start();
assert.deepEqual(sidecarStart.srtFiles, [{
  name: 'roteiro.srt', content: '1\n00:00:00,000 --> 00:00:01,000\nCena documental',
}], 'the selected folder returns bounded SRT content without exposing its path');
