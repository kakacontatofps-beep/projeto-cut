...
export function pruneSemanticVectors(
  ...
  // Pruning is housekeeping. When sqlite-vec is unavailable, the renderer uses
  // its IndexedDB fallback, so cleanup must not turn into a noisy IPC failure.
  const db = openConnection();
  if (!db) return { staleModelRemoved: false, staleSourceRemoved: false };
